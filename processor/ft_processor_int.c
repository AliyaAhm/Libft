...
static int  ft_ret_part_int(char *string, int buff_i, t_flags flags)
{
    ...
    len = 0;
    if (buff_i < 0 && flags.precision >= 0 && buff_i > -2147483648)
        ft_putchar('-');
...
static int ft_part_int(char *string, int buff_i, t_flags flags)
{
    int len;

    len = 0;
    if (flags.minus == 1)
        len += (int)ft_ret_part_int(string, buff_i, flags);
    if (flags.precision >= 0 && (size_t)flags.precision < ft_strlen(string))
...
string = ft_itoa(i);
len = (int)ft_part_int(string, buff_i, flags);
free(string);
return(len);
}